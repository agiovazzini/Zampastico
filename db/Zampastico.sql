...
CREATE TABLE IF NOT EXISTS ImmagineProdotto(
...
CREATE TABLE IF NOT EXISTS InventarioStock(
    ...
    CONSTRAINT prodotto_disponibile CHECK (quantita >= 0)
);
...
CREATE TABLE IF NOT EXISTS Recensione(
    ...
    FOREIGN KEY (id_utente) REFERENCES Utente(id_utente) ON DELETE CASCADE,
    FOREIGN KEY (id_prodotto) REFERENCES Prodotto(id_prodotto) ON DELETE CASCADE,
    CONSTRAINT recensione_utente UNIQUE (id_utente, id_prodotto)
);
...
CREATE TABLE IF NOT EXISTS VoceWishlist(
    id_voceWishlist INT AUTO_INCREMENT PRIMARY KEY,
    id_wishlist INT NOT NULL,
    id_varianteProdotto INT NOT NULL,
    FOREIGN KEY (id_wishlist) REFERENCES Wishlist(id_wishlist) ON DELETE CASCADE,
    FOREIGN KEY (id_varianteProdotto) REFERENCES VarianteProdotto(id_varianteProdotto) ON DELETE CASCADE,
    CONSTRAINT wishlist_variante UNIQUE (id_wishlist, id_varianteProdotto)
);
...
CREATE TABLE IF NOT EXISTS VoceOrdine(
    id_voceOrdine INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS Pagamento(
    ...
    stato ENUM('completato', 'fallito', 'rimborsato', 'in elaborazione') DEFAULT 'in elaborazione' NOT NULL,
